...
(function(){
  ...
  function filteredLoads(){
    ...
    if(statusFilter!=='all')list=list.filter(load=>String(load.keep==null?'pending':load.keep)===statusFilter);
  ...
  function renderGroupedLoads(){
    ...
    el.querySelectorAll('[data-group-status]').forEach(select=>{
      ...
      select.onchange=async event=>{
        ...
        try{await save();toast(`Status changed to ${statusFor(load)[1]||'None'}`);}catch(error){load.keep=previous;renderStatusChips();renderPowderChips();renderLoads();toast('Could not change status');}
      };
    });
